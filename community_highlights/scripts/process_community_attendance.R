...
# ------------------------------------------------------------
# Name alias / identity reconciliation
# ------------------------------------------------------------
#
# Single source of truth:
#   community_highlights/name_aliases.csv
#
# Required columns:
...
#   exclude
#
# `alias` can be a literal Teams display name OR a previously normalized key.
# Both are normalized through clean_name_key() before matching.

alias_lookup_file <- file.path(
  community_dir,
  "name_aliases.csv"
)

if (!file.exists(alias_lookup_file)) {
  readr::write_csv(
    tibble(
      alias = character(),
...
name_reconciliation <- readr::read_csv(
  alias_lookup_file,
  show_col_types = FALSE
)

required_alias_cols <- c("alias", "canonical_name")

missing_alias_cols <- setdiff(
  required_alias_cols,
  names(name_reconciliation)
)

if (length(missing_alias_cols) > 0) {
  stop(
    "name_aliases.csv is missing required columns: ",
    paste(missing_alias_cols, collapse = ", ")
  )
}

if (!"exclude" %in% names(name_reconciliation)) {
  name_reconciliation$exclude <- FALSE
}

name_reconciliation <- name_reconciliation |>
  transmute(
    alias = as.character(alias),
    canonical_name = na_if(str_squish(as.character(canonical_name)), ""),
    exclude = replace_na(as.logical(exclude), FALSE),
    observed_key = clean_name_key(alias),
...
alias_conflicts <- name_reconciliation |>
  group_by(observed_key) |>
  summarise(
    n_canonical = n_distinct(
      paste0(coalesce(canonical_name, "<NA>"), "|", exclude)
    ),
    .groups = "drop"
  ) |>
...
if (nrow(alias_conflicts) > 0) {
  stop(
    "Conflicting mappings found in name_aliases.csv for: ",
    paste(alias_conflicts$observed_key, collapse = ", ")
  )
}

name_reconciliation <- name_reconciliation |>
  distinct(observed_key, .keep_all = TRUE)
