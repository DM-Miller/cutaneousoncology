...
author: "Sophia Z. Shalhout^1^, Justine Cohen^2^, Khalid Garmen^3^, Emily Kim^4^, Sonia Cohen^1^ David M. Miller^1^"
date: "2022-11-21"
format: docx 
output: true
link-citations: yes
...
image: files/november_2022/november_2022_featured_image.jpg
categories: [2022, Merkel Cell Carcinoma, Ipi/Nivo]
description: In this `Perspectives on the Science` piece we reflect on the impact of the recently published article "Combined Nivolumab and Ipilimumab with or without Stereotactic Body Radiation Therapy for Advanced Merkel cell carcinoma - A Randomised, Open Label, Phase 2 Trial". 
---
...
![](files/november_2022/Figure2.jpg)

![](files/november_2022/Figure3.jpg)
...
cut.onc.non.adj <- dt |> 
  select(name, Dz, N, Indication, Indication_brief, END, SingleArmNum_N_Y) |> 
  filter(!str_detect(dt$Indication, "adjuvant"))

cut.onc.non.adj.1 <- cut.onc.non.adj |> 
  filter(
    !str_detect(cut.onc.non.adj$Indication_brief, "adjuvant")
  )

cut.onc.non.adj.2 <- cut.onc.non.adj.1 |> 
  filter(
    !str_detect(cut.onc.non.adj.1$Indication_brief, "Adjuvant")
  )  

cut.onc.non.adj.3 <- cut.onc.non.adj.2 |> 
  filter(
    !str_detect(cut.onc.non.adj.2$Indication_brief, "Week Dosing")
  )  

cut.onc.non.adj.4 <- cut.onc.non.adj.3 |> 
  filter(Dz != "AK")  

cut.onc.non.adj.5 <- cut.onc.non.adj.4 |> 
    filter(
    !str_detect(cut.onc.non.adj.4$Indication_brief, regex("weeks dosing", ignore_case = TRUE))
  )

cutaneous.onc.n <- cut.onc.non.adj.5 |> drop_na() |> select(N)
cutaneous.onc.n.1 <- mean(cutaneous.onc.n$N)
cutaneous.onc.50 <- cutaneous.onc.n |> filter(N <= 50)
cutaneous.onc.100 <- cutaneous.onc.n |> filter(N <= 100)
```

We began Journal Club on November 7^th^, 2022 by administering a poll among the attendees via [poll everywhere](https://www.polleverywhere.com/). Our goal was to conduct a brief survey before and after journal club discussions to determine the current practices of clinicians participating in the dialogue and whether review/discussion of the article would be 'practice-changing.' The following reflects the survey results of the poll administered at the start of the meeting:

![](files/november_2022/q1.jpg)

![](files/november_2022/q2.jpg)

![](files/november_2022/q3.jpg)

![](files/november_2022/q4.jpg)

The following reflects the survey results of the poll after our discussion and article review:

![](files/november_2022/q5.jpg)

![](files/november_2022/q6.jpg)
...
2.  Pennsylvania Hospital and University of Pennsylvania, Philadelphia PA
3.  Visiting Fellow in the Dermatology Branch at the National Institute of Arthritis and Musculoskeletal and Skin Diseases\
