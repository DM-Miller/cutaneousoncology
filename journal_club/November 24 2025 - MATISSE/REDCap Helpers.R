...
plot_checkbox <- function(checkbox_data, var_name, dict_path = NULL, 
  ...
  if (length(question_title) == 0) stop("Variable not found: ", var_name)
  
  respondent_total <- checkbox_data$.respondent_total[1]
  
  plot_data <- checkbox_data %>%
    dplyr::mutate(
      prop = round(n_selected / respondent_total * 100)
    )
  
  # Custom order
  if (!is.null(custom_order)) {
    plot_data <- plot_data %>%
      dplyr::mutate(label = factor(label, levels = rev(custom_order), ordered = TRUE))
  } else {
    plot_data <- plot_data %>%
