...
Nrisk_low_up <- function(
  ...
  lower1 <- sapply(1:(length(t.risk1) - 1), function(i) {
    inds <- which((t.risk1[i] <= t.S1) & (t.S1 < t.risk1[i + 1]))
    if (length(inds)==0) NA else min(inds)
  })
  upper1 <- sapply(1:(length(t.risk1) - 1), function(i) {
    inds <- which((t.risk1[i] <= t.S1) & (t.S1 < t.risk1[i + 1]))
  ...
  # Mark invalid intervals
  valid_main <- !(is.na(lower1[1:(length(lower1)-1)]) | is.na(upper1[1:(length(upper1)-1)]))
  
  # Always keep the last interval
  lower1 <- c(lower1[valid_main], lower1[length(lower1)])
  upper1 <- c(upper1[valid_main], upper1[length(upper1)])
  t.risk1 <- c(t.risk1[valid_main], t.risk1[length(t.risk1)])
  n.risk1 <- c(n.risk1[valid_main], n.risk1[length(n.risk1)])
  
  cat("n.risk length:", length(n.risk1), "\n")
  cat("lower length:", length(lower1), "\n")
  cat("upper length:", length(upper1), "\n")
...
reconKMGuyot <- function(
  ...
  if (n.int > 1) {
    for (i in 1:(n.int - 1)) {
      ...
      while ((n.hat[lower[i + 1]] > n.risk[i + 1]) || 
        ...
        for (k in lower[i]:upper[i]) {
          ...
          ratio <- S[k] / KM.hat[last]
          
          # 🟢 Diagnostic print
          cat(sprintf("Interval %d, k=%d, S[k]=%.4f, KM.hat[last]=%.4f, ratio=%.4f, n.hat=%.2f\n",
                      i, k, S[k], KM.hat[last], ratio, n.hat[k]))
          
          if (is.na(ratio) || ratio > 1) ratio <- 1
          d[k] <- if (i == 1 && k == lower[i]) 0 else round(n.hat[k] * (1 - ratio))
    ...
    # 🟢 Insert this diagnostic summary block here:
    ...
    # ---------------------------------------
    # Scale d vector to match total events
    # ---------------------------------------
    # Assign target events
    target_events <- tot.events
    
    # Compute total assigned events
    total_assigned_events <- sum(d)
    
    # Compute scaling factor
    if (total_assigned_events == 0) {
      scaling_factor <- 1
    } else {
      scaling_factor <- target_events / total_assigned_events
    }
    
    # Scale event counts proportionally (without rounding yet)
    d_scaled <- d * scaling_factor
    
    # Round down all but last
    d_rounded <- floor(d_scaled)
    
    # Compute how many events remain to be assigned
    remaining_events <- target_events - sum(d_rounded)
    
    # Assign remaining events to the timepoints with largest decimal remainder
    # (to keep proportionality)
    remainder_order <- order(d_scaled - d_rounded, decreasing = TRUE)
    
    # Increment counts on top 'remaining_events' bins
    ...
    cat("Target events:", target_events, 
        "Assigned events after scaling:", sum(d_rounded), "\n")
...
process_KM_data <- function(
  ...
  return(list(
    dat = cbind(t.S, S, n.hat[1:n.t], d, cen),
    ipd = IPD,
