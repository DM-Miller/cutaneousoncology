...
<style>
  ...
  .attendance-details .attendance-note {
    margin: 0;
    padding: .85rem 1rem;
...
::: {.section-intro}
Teams reconnects are reconciled before display, and abbreviated attendee names are normalized in the attendance-processing script. The shared Replimune conference-room connection is represented below by the individual Replimune colleagues whose participation was confirmed. The attendance roster is limited to names and institutional affiliations.
:::
...
This roster records attendance and institutional affiliation only.
</div>
...
# Portable across Journal Club and Research Forum recaps.
# ============================================================
...
# Reconcile a few known display-name variants before lookup.
name_reconciliation <- tibble::tribble(
  ...
  "replimuneteam",       "Replimune team"
)

# First + last name key. This intentionally ignores middle initials,
# credentials, and the malformed "IV" that previously appeared in
# William Benjamin's display name.
...
# The IGNYTE Teams export preserved the Replimune participants as a shared
# conference-room connection rather than as separate person-level rows.
# These three colleagues were independently confirmed to have participated,
# so represent the room connection with their individual names in the
# public attendance roster.
supplemental_industry_attendees <- tibble::tribble(
  ~raw_name,
  "Adam Whalley",
  "Adam Yakovich",
  "Jeannie Hou"
...
roster_source <- people |>
  transmute(
    raw_name = Name
  ) |>
  # Do not display the generic shared-room label once the confirmed
  # individual participants have been added.
  filter(
    clean_name_key(raw_name) != "replimuneteam"
  ) |>
  bind_rows(
    supplemental_industry_attendees
  ) |>
  distinct() |>
  mutate(
    observed_key = clean_name_key(raw_name)
  )
...
cat(
  paste0(
    '<div class="roster-summary">',
      '<strong>', nrow(roster), ' attendance records</strong>',
