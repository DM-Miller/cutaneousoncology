...
standardize_acronyms <- function(x) {
  ...
  x %>%
    ...
    stringr::str_replace_all("\\bMcc\\b", "MCC") %>%
    stringr::str_replace_all("\\bRt\\b", "RT") %>%
    stringr::str_replace_all("\\bApp\\b", "APP") %>%
    stringr::str_replace_all("\\bSlnb\\b", "SLNB") %>%
    stringr::str_replace_all("\\bPd-1\\b", "PD-1") %>%
    stringr::str_replace_all("\\bPd-L1\\b", "PD-L1") %>%
    stringr::str_replace_all("\\bEnt\\b", "ENT") |> 
    stringr::str_replace_all("\\bCm\\b", "cm") |> 
...
plot_checkbox <- function(checkbox_data, var_name, dict_path = NULL, 
  ...
  subtitle_text <- if (!is.null(subtitle) && nzchar(subtitle)) {
    str_wrap(subtitle, width = subtitle_width)
  } else {
    NULL
  }
...
safe_plot_question <- function(q, dt_clean, checkbox_data, custom_order = NULL, ...) {
  
  var_name <- q$var
  
  # If variable is missing entirely
  if (is.null(var_name) || !var_name %in% names(dt_clean)) {
    return(
      ggplot2::ggplot() +
        ggplot2::annotate(
          "text", x = 0.5, y = 0.5,
          label = paste0("No data available for: ", var_name),
          size = 8
        ) +
        ggplot2::theme_void()
    )
  ...
  if (length(vals) == 0) {
    return(
      ggplot2::ggplot() +
        ggplot2::annotate(
          "text", x = 0.5, y = 0.5,
          label = "No responses submitted yet.",
          size = 8
        ) +
        ggplot2::theme_void()
    )
  ...
  # If custom_order accidentally removes all observed values, fall back to default order
  if (!is.null(custom_order)) {
    if (length(intersect(unique(vals), custom_order)) == 0) {
      message("Custom order did not match observed values for ", var_name)
      message("Observed values: ", paste(unique(vals), collapse = " | "))
      custom_order <- NULL
    }
  }
